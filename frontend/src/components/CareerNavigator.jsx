...
const CareerNavigator = () => {
  ...
  const getCardClass = (financial) => {
    if (financial.status === 'within_budget') return 'border-l-4 border-emerald-500 bg-gradient-to-r from-[#1A2A4A] to-emerald-900/20';
    if (financial.status === 'small_gap' || financial.status === 'medium_gap') return 'border-l-4 border-blue-500 bg-gradient-to-r from-[#1A2A4A] to-blue-900/20';
    return 'border-l-4 border-purple-500 bg-gradient-to-r from-[#1A2A4A] to-purple-900/20';
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#0A1628] via-[#0F1F3A] to-[#1A2A4A]">
      <div className="max-w-7xl mx-auto px-4 md:px-6 py-6 md:py-8 space-y-8">
        {/* Hero Header - Updated to match theme */}
        <div className="rounded-2xl p-6 md:p-8 text-white shadow-xl bg-gradient-to-r from-[#0A1628] via-[#1A4A6F] to-[#0A1628] border border-indigo-500/20">
          ...
          <div className="flex flex-wrap gap-2 mt-4">
            <span className="bg-indigo-500/20 px-3 py-1.5 rounded-full text-xs flex items-center gap-1 text-indigo-200">
        ...
        {/* Budget Info Banner - Updated colors */}
        <div className="bg-amber-500/10 border-l-4 border-amber-500 p-4 rounded-r-xl backdrop-blur-sm">
          <div className="flex items-start gap-3">
            <Lightbulb size={20} className="text-amber-400 mt-0.5 flex-shrink-0" />
            <div className="text-sm">
              <p className="font-semibold text-amber-300">💡 Understanding Your Budget of ₹{budget} Lakhs</p>
              <p className="text-amber-200/80 mt-1">
                This is your upfront amount for 1st year fees + living expenses. Many students cover gaps through:
                <span className="font-medium"> education loans, scholarships (20-100% tuition), and part-time work (₹8-15 Lakhs/year)</span>.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-[#1A2A4A]/60 backdrop-blur-sm rounded-2xl shadow-xl border border-indigo-500/20 overflow-hidden">
          <div className="bg-[#0F1F3A]/50 px-6 py-4 border-b border-indigo-500/20">
          ...
          <div className="p-6">
            <div className="grid md:grid-cols-2 gap-6">
              <div>
                <label className="block text-sm font-semibold text-indigo-200 mb-2 flex items-center gap-1">
                  <Heart size={14} className="text-rose-400" /> Field of Interest *
                </label>
                <input
                  type="text"
                  placeholder="e.g., Computer Science, MBA, Data Science"
                  className="w-full p-3 bg-[#0A1628]/50 border border-indigo-500/30 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none text-white placeholder:text-indigo-400/50"
                  value={interest}
              ...
              <div>
                <label className="block text-sm font-semibold text-indigo-200 mb-2 flex items-center gap-1">
                  <DollarSign size={14} className="text-emerald-400" /> Your Budget (INR Lakhs) - 1st Year
                </label>
                <input
                  type="number"
                  className="w-full p-3 bg-[#0A1628]/50 border border-indigo-500/30 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none text-white"
                  value={budget}
                  onChange={(e) => setBudget(e.target.value)}
                />
                <p className="text-xs text-indigo-400/60 mt-1">*Your available funds for tuition + living expenses (1st year)</p>
              </div>

              <div>
                <label className="block text-sm font-semibold text-indigo-200 mb-2 flex items-center gap-1">
                  <Globe size={14} className="text-sky-400" /> Preferred Country
                </label>
                <select
                  className="w-full p-3 bg-[#0A1628]/50 border border-indigo-500/30 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none text-white"
              ...
              <div>
                <label className="block text-sm font-semibold text-indigo-200 mb-2 flex items-center gap-1">
                  <Target size={14} className="text-indigo-400" /> Academic Score: <span className="font-bold text-white">{academicScore}</span>%
                </label>
                <input
                  type="range"
                  min="50"
            ...
            <button
              ...
              disabled={loading}
              className="mt-8 w-full bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 text-white py-3.5 rounded-xl font-bold flex items-center justify-center gap-2 text-lg disabled:opacity-50 transition-all hover:scale-[1.02]"
            >
              {loading ? (
                <>
        ...
        {/* Results Section */}
        {loading && (
          <div className="bg-[#1A2A4A]/60 backdrop-blur-sm rounded-2xl p-12 text-center border border-indigo-500/20">
            <Brain size={48} className="text-indigo-400 animate-pulse mx-auto mb-4" />
            <p className="text-indigo-300">AI is analyzing {universitiesDB.length} universities based on your profile...</p>
          </div>
        )}
...
// University Card Component - Dark theme
const UniversityCard = ({ uni, onClick, getCardClass, getProbClass, getProbText }) => {
  return (
    <div
      onClick={onClick}
      className={`bg-[#1A2A4A]/80 backdrop-blur-sm rounded-xl shadow-lg border border-indigo-500/20 overflow-hidden cursor-pointer transition-all duration-300 hover:shadow-xl hover:-translate-y-1 ${getCardClass(uni.financial)}`}
    >
      <div className="p-4">
        ...
        <div className="flex justify-between text-sm mb-3">
          <span className="flex items-center gap-1 text-indigo-300"><Tag size={12} className="text-emerald-400" />₹{uni.costLakhs}L/year</span>
          <span className="flex items-center gap-1 text-indigo-300"><CalendarIcon size={12} />{uni.deadline}</span>
        </div>
        ...
        <p className="text-xs text-indigo-300/80">{uni.financial.message.substring(0, 80)}...</p>
      </div>
    </div>
  );
};

// University Modal Component - Dark theme
const UniversityModal = ({ uni, onClose, getProbClass, getProbText }) => {
  return (
    <div className="fixed inset-0 bg-[#0A1628]/95 backdrop-blur-md flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="bg-[#1A2A4A] rounded-2xl max-w-md w-full max-h-[90vh] overflow-y-auto border border-indigo-500/30 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="bg-[#0F1F3A] px-6 py-4 rounded-t-2xl border-b border-indigo-500/20 flex justify-between items-center sticky top-0">
          <h3 className="text-xl font-bold text-white">{uni.name}</h3>
          <button onClick={onClose} className="text-indigo-400 hover:text-indigo-200 transition">
            <X size={20} />
          </button>
        </div>
        <div className="p-6 space-y-4">
          <div className="grid grid-cols-2 gap-3 text-sm">
            <div><span className="font-semibold text-indigo-300">Country:</span></div><div className="text-white">{uni.country}</div>
            <div><span className="font-semibold text-indigo-300">Course:</span></div><div className="text-white">{uni.course}</div>
            <div><span className="font-semibold text-indigo-300">Admission Chance:</span></div>
            ...
            <div><span className="font-semibold text-indigo-300">Annual Cost:</span></div><div className="text-white">₹{uni.costLakhs} Lakhs</div>
            <div><span className="font-semibold text-indigo-300">Deadline:</span></div><div className="text-white">{uni.deadline}</div>
          </div>
          ...
          <div className="bg-indigo-500/10 p-4 rounded-xl border border-indigo-500/20">
            ...
            <div className="space-y-2 text-sm">
              <p><span className="font-semibold text-indigo-300">Status:</span> <span className={`px-2 py-0.5 rounded text-white text-xs ${uni.financial.badgeClass}`}>{uni.financial.badge}</span></p>
              <p className="text-indigo-200">{uni.financial.message}</p>
              {uni.financial.loanRequired > 0 && (
                <>
                  <p className="text-indigo-200"><span className="font-semibold text-indigo-300">Loan Required:</span> ₹{uni.financial.loanRequired} Lakhs</p>
                  <p className="text-indigo-200"><span className="font-semibold text-indigo-300">Monthly EMI (5 years):</span> ~₹{uni.financial.monthlyEmi.toLocaleString()}</p>
                </>
              )}
              <p className="text-indigo-200"><span className="font-semibold text-indigo-300">Part-time Work:</span> {uni.partTimeWork}</p>
              <p className="text-emerald-400 font-medium mt-2">💡 {uni.financial.recommendation}</p>
            </div>
          </div>

          <div className="bg-amber-500/10 p-3 rounded-xl border border-amber-500/20">
            <p className="text-sm flex items-start gap-2 text-amber-300">
          ...
          <div className="bg-emerald-500/10 p-3 rounded-xl border border-emerald-500/20">
            <p className="text-sm flex items-start gap-2 text-emerald-300">
        ...
        <div className="px-6 pb-6">
          <button onClick={onClose} className="w-full bg-gradient-to-r from-indigo-500 to-purple-600 text-white py-2.5 rounded-xl font-semibold hover:shadow-lg transition-all">
            Close
          </button>
        </div>
      </div>
